...
namespace gbc
{
        ...
        opcode_map =
            {
                ...
                // LD into register A
                //{0x36, OpcodeCommand("LD to HL", std::bind(&Opcode::Ld_A, this, &(CPU->BC)), 8)},
                //{0x36, OpcodeCommand("LD to HL", std::bind(&Opcode::Ld_A, this, &(CPU->DE)), 8)},

                // LD nn,n I think?
                {0x06, OpcodeCommand("Ldnn,n", std::bind(&Opcode::LdWriteByte, this, &(CPU->B)), 8)},
                {0x0E, OpcodeCommand("Ldnn,n", std::bind(&Opcode::LdWriteByte, this, &(CPU->C)), 8)},
                {0x16, OpcodeCommand("Ldnn,n", std::bind(&Opcode::LdWriteByte, this, &(CPU->D)), 8)},
                {0x1E, OpcodeCommand("Ldnn,n", std::bind(&Opcode::LdWriteByte, this, &(CPU->E)), 8)},
                {0x26, OpcodeCommand("Ldnn,n", std::bind(&Opcode::LdWriteByte, this, &(CPU->H)), 8)},
                {0x2E, OpcodeCommand("Ldnn,n", std::bind(&Opcode::LdWriteByte, this, &(CPU->L)), 8)},
                // LD
                ...
                // JP HL
                {0xDA, OpcodeCommand("JP (HL)", std::bind(&Opcode::JpHL, this), 4)},
                // JR n
                {0xDA, OpcodeCommand("JR n", std::bind(&Opcode::JrN, this), 8)},
                // JR cc,n
    ...
    void Opcode::IncN(register8_t *r1)
    {
        ...
        CPU->FLAGS.SetSubtractFlag(false);
        CPU->FLAGS.SetHalfCarryFlag((0x10 & r1->value()) != 0);
    }
    ...
    void Opcode::DecN(register8_t *r1)
    {
        ...
        CPU->FLAGS.SetZeroFlag(r1->value() == 0x0);
        CPU->FLAGS.SetSubtractFlag(true);
        CPU->FLAGS.SetHalfCarryFlag((r1->value() & 0xF) < 0);
    }
    ...
    void Opcode::SwapHL()
    {
        register8_t byte = gbc::Ram::Instance()->ReadByte(CPU->HL.value());
        Swap(&byte);
    ...
    void Opcode::Rla(register8_t *r1)
    {
    ...
    void Opcode::RrA(register8_t *r1)
    {
        bool carry = CPU->FLAGS.CarryFlag();
        bool bit_0 = r1->BitAtLSB(0);
    ...
    void Opcode::Sla(register8_t *r1)
    {
    ...
    void Opcode::Sra(register8_t *r1)
    {
        bool bit_0 = r1->BitAtLSB(0);
    ...
    void Opcode::Srl(register8_t *r1)
    {
        bool bit_0 = r1->BitAtLSB(0);
        bool msb = r1->BitAtLSB(7);
        r1->mValue = r1->value() >> 1;
    ...
    void Opcode::RlcA_HL()
    {
        register8_t tmp = register8_t(Ram::Instance()->ReadByte(CPU->HL.value()), "HL Byte");
        RlcA(&tmp);
    ...
    void Opcode::JpHL()
    {
        CPU->PC.mValue = CPU->HL.value();
    }

    void Opcode::JrN()
    {
        int8_t read = static_cast<int8_t>(CPU->GetByteFromPC());
        CPU->PC.mValue = CPU->PC.value() + read;
    }
    ...
    void Opcode::CallNN()
    {
        register16_t tmp = CPU->PC.mValue;
        Push(&tmp);
        uint16_t read = CPU->GetWordFromPC();
        CPU->PC.mValue = read;
    }
    ...
    void Opcode::BitBR(byte b, register8_t *r1)
    {
    ...
    void Opcode::SetBR(byte b, register8_t *r1)
    {
        r1->SetBitLSB(b);
    }
    ...
    void Opcode::ResBR(byte b, register8_t *r1)
    {
        r1->ResetBitLSB(b);
    }
